...
if($_POST) {
	...
	if(in_array($type, array('')) && in_array($type_banner, array(''))){
	    $sql = "UPDATE breaking_news SET reporter='$reporter', title='$title', description='$description', type='$type', category='$category', video_url='$video_url', status='$status' WHERE id='$id'";
		...
		if(is_uploaded_file($_FILES['userImage']['tmp_name']) && is_uploaded_file($_FILES['userImage_banner']['tmp_name'])) {
			if(move_uploaded_file($_FILES['userImage']['tmp_name'], $url) && move_uploaded_file($_FILES['userImage_banner']['tmp_name'], $url_banner)) {
				...
				$sql = "UPDATE breaking_news SET reporter='$reporter', title='$title', description='$description', type='$type', category='$category', video_url='$video_url', image_square='$url', image_banner='$url_banner', status='$status' WHERE id='$id'";
